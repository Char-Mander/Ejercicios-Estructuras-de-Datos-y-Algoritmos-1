...
Luego comprobamos si el total que llevamos es mayor que la mejor solución. Si no lo es, comprobamos si estamos en el final (k==matriz.size()-1). Si está, comprobamos si el total es menor que la mejor solución. Si lo es,
...
void resolver(const std::vector<std::vector<int>> &matriz, std::vector<bool> &usadas, std::vector<int> &sol, int &total, int k, int &mejor) {
	...
	for (int trabajo = 0; trabajo<matriz.size(); trabajo++) {
		if (!usadas[trabajo]) {
			...
			if (total < mejor) { //solo continuamos si el total sigue siendo menor que la mejor solución
				if (k == matriz.size() - 1) {
...
#ifndef DOMJUDGE
	std::ifstream in("//psf/Home/Desktop/Trabajos y más/Universidad/2º E/EDA - 2º E/Ejercicios EDA/EDA LAB P33/EDA LAB P33/prueba.txt");
