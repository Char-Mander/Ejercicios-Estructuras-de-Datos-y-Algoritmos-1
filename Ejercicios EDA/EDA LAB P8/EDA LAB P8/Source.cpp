...
// Habiendo cogido por teclado n, primero se comprueba si el vector es ascendente. si lo es, p sería v.size()-1 (y también coge el máximo del vector)
...
// Tiene un coste lineal,  ya que recorre el vector desde el principio y desde el final,
//hasta que la variable ini es mayor que fin
...
int resolver(std::vector<int> const &v) {
	...
	if (ascendente || max1==v[0] || v[0]>= v[v.size()-1])
		p = v.size()-1;
	else {
		while (fin > ini) {
			if (max2 < v[ini])
...
#ifndef DOMJUDGE
	std::ifstream in("//psf/Home/Desktop/Trabajos y más/Universidad/2º E/EDA - 2º E/Ejercicios EDA/EDA LAB P8/EDA LAB P8/prueba.txt");
